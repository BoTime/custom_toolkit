...
## Locating these scripts

`$SKILL_DIR` below means this skill's own directory — the absolute path from
the `Base directory for this skill:` line the harness prefixed to this skill
...
"$SKILL_DIR/scripts/start-server.sh" --project-dir /path/to/project --open --foreground
```
...
Paths below are relative to `$SKILL_DIR/`.
